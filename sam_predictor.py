...
VITB_CHECKPOINT = os.environ.get("VITB_CHECKPOINT", "sam_vit_b_01ec64.pth")
VITL_CHECKPOINT = os.environ.get("VITL_CHECKPOINT", "sam_vit_l_0b3195.pth")
...
class SAMPredictor(object):
    ...
    def __init__(self, model_choice):
        ...
        elif model_choice == 'SAM_B':
            from segment_anything import SamPredictor, sam_model_registry

            self.model_checkpoint = VITB_CHECKPOINT
            if self.model_checkpoint is None:
                raise FileNotFoundError("VITB_CHECKPOINT is not set: please set it to the path to the SAM checkpoint")

            logger.info(f"Using SAM_B checkpoint {self.model_checkpoint}")
            reg_key = "vit_b"

        elif model_choice == 'SAM_L':
            from segment_anything import SamPredictor, sam_model_registry

            self.model_checkpoint = VITL_CHECKPOINT
            if self.model_checkpoint is None:
                raise FileNotFoundError("VITL_CHECKPOINT is not set: please set it to the path to the SAM checkpoint")

            logger.info(f"Using SAM_L checkpoint {self.model_checkpoint}")
            reg_key = "vit_l"
    ...
    def predict(
        ...
        elif self.model_choice in ('SAM', "SAM_L", "SAM_B", 'MobileSAM'):
